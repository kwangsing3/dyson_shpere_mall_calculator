import {
    電力感應塔, 傳送帶MKI, 傳送帶MKII, 傳送帶MKIII, 抽水站, 無線輸電塔, 蓄電器, 電磁軌道彈射器, 太陽能板,
    地熱發電站,
    射線接收站,
    人造恆星,
    四向分流器,
...
import { Building, Material, Unit } from "./interface";
    ...
    req.forEach(ele => {
        if (ele.recipe.length > 0) {
            ele.recipe.forEach(re => {
                if (!(re as Building).calulateSKIP && !(re as Material).ignore)
...
function AnalyzeEle(data: { [key: string]: number }) {
    ...
    let st = ''
    let extraInfo = `補充- 由於以下物品比重太低，建議另外拉出特供的路線\n`;
    let r = 1;
    let mx = 0
    for (const index in analys) {
        ...
        key.forEach(element => {
            ...
            const tr = `【${per < 1 ? 1 : per}】  ${persentFunc(element.amount, toal)}      ${element.name}:${element.amount}`;
            if (per < 2) {
                extraInfo += tr + '\n'
            } else {
                t += per
        ...
        st += d
        st += `理論上限 ${t}/${speed} 爪速/秒` + `    環上總共 ${toal}項 \n`
    }
    //
    //報告
    const str = `理論最內側 (最短軌道) 長度至少需要大於` + mx + `格數才能同時放下所有請求物\n`
    return st + str + extraInfo
}
...
function splitIntoGroups(arr: [name: string, amount: number][], numGroups: number) {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        if (targetSum < num[1]) {
            targetSum = num[1]
            for (const index in groups) groups[index] = [];//清空
            currentSum = 0
            currentGroup = 0
            i = -1
            continue;
